...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-semibold">Volleyball Scorer</h2>
        <a href="{{ route('admin.volleyball.matches.index') }}" class="text-sm text-indigo-600 hover:underline">Back to Matches</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6 space-y-6">
        ...
        @php $sets = $match->set_scores ?? []; @endphp
        @if(!empty($sets))
            <div class="text-sm text-gray-600">
                <div class="font-semibold mb-1">Set Scores</div>
                <div class="flex flex-wrap gap-2">
                    @foreach($sets as $i => $s)
                        <span class="px-2 py-1 rounded bg-gray-100">Set {{ $i+1 }}: {{ $s['a'] ?? 0 }} - {{ $s['b'] ?? 0 }}</span>
                    @endforeach
                </div>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
            <div class="text-center">
                ...
                <div class="mt-4 flex justify-center gap-2">
                    <form method="POST" action="{{ route('admin.volleyball.matches.score', $match) }}">@csrf
            ...
            <div class="text-center">
                ...
                <form method="POST" action="{{ route('admin.volleyball.matches.setStatus', $match) }}" class="inline-flex items-center gap-2">@csrf
