...
@section('title', 'Edit Volleyball Match')
...
<div class="py-10 bg-gray-50 min-h-screen">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-xl rounded-2xl p-8 border border-gray-100">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-6">Edit Volleyball Match</h2>

            <form action="{{ route('admin.volleyball.matches.update', $match->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Set Scores (JSON)</label>
                    <textarea name="set_scores" rows="3" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ $match->set_scores ? json_encode($match->set_scores) : '' }}</textarea>
                </div>
