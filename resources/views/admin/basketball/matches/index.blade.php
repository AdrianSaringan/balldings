...
@section('title', 'Basketball Matches')

@section('content')
<div class="py-8 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Basketball Matches</h1>
        ...
        <div class="bg-white shadow-lg rounded-2xl p-6">
            ...
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('admin.basketball.matches.create') }}" 
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse($matches as $match)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-3">{{ \Carbon\Carbon::parse($match->match_date)->format('M d, Y') }}</td>
                                <td class="px-6 py-3">{{ $match->venue }}</td>
                                <td class="px-6 py-3">{{ ucfirst($match->status) }}</td>
                                <td class="px-6 py-3 font-semibold">{{ $match->winner ?? 'N/A' }}</td>
                                <td class="px-6 py-3 flex gap-2">
                                    <a href="{{ route('admin.basketball.matches.edit', $match->id) }}" 
                                       class="px-3 py-1 bg-blue-600 text-white rounded-full text-xs hover:bg-blue-700 transition">
                                        Edit
                                    </a>
                                    <form action="{{ route('admin.basketball.matches.destroy', $match->id) }}" 
                                          method="POST" 
