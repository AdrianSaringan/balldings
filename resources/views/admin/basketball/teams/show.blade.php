...
<div class="py-8 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-800">Basketball Team: {{ $team->team_name }}</h1>
            <a href="{{ route('admin.basketball.teams.index') }}" class="px-4 py-2 bg-gray-700 text-white rounded-md text-sm hover:bg-gray-800">← Back to Teams</a>
        </div>
        ...
        <div class="bg-white shadow-xl rounded-2xl p-6">
            ...
            <div class="mb-6">
                <form action="{{ route('admin.basketball.teams.players.add', $team->id) }}" method="POST" class="flex items-end gap-3">
                    @csrf
                    <div class="flex-1">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @forelse($players as $p)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-3">
                                    <form action="{{ route('admin.basketball.teams.players.remove', [$team->id, $p->id]) }}" method="POST" onsubmit="return confirm('Remove this player from the team?');">
                                        @csrf
                                        @method('DELETE')
